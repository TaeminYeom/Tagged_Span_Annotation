# annotation -> annotated_translation
...
class ErrorAnnotation(BaseModel):
    ...
    category: str  # "category/subcategory" format

class ErrorResponse(BaseModel):
    """
    Response model for translation quality evaluation with tagged segments.
    
    STRICT JSON FORMAT - NO COMMENTS OR EXPLANATIONS ALLOWED
    
    Structure:
    {
        "annotated_translation": "Text with <v0>error1</v0> and <v1>error2</v1>",
        "errors": [
...
def build_prompt(src_line: str, src_lang: str,
    ...
    """
    번역문에 <v0>, <v1> 태그로 오류 구간을 표시하고 JSON으로 각 태그의 정보를 반환하는 방식
    """
...
async def query_openai_async(prompt, client: AsyncOpenAI, model, semaphore: Semaphore, max_retries=3):
    ...
    async with semaphore:
        ...
        return {"annotated_translation": "", "errors": [], "error": "Max retries exceeded"}
